...
class UserController extends Controller
{
    ...
    function sendActivateMail($account) {
        ...
        if($user->save()) {
            Mail::send('emails.mail', ['user' => $user], function ($m) use ($account) {
                ...
                $m->to($account->Email, $account->FirstName . " " . $account->LastName)->subject('Account activation');
            });
        }
    }
    ...
    public function mailen()
    {
        ...
        Mail::send('emails.mail', ['user' => $user], function ($m) use ($user) {
            ...
            $m->to('stevenschenk74@hotmail.com', $user->FirstName . " " . $user->LastName)->subject('Testing design');
        });
    }
